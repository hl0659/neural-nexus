"""
Neural Nexus v3.0 - NEXUS Parallel Collection Runner
...
from services.nexus.activator import NexusActivator
from database.operations.queue_ops import mark_queue_processing, mark_queue_complete, mark_queue_error
from database.operations.status_ops import mark_service_active, update_service_status


class ParallelCollectionRunner:
    """Runs NEXUS collection across all regions in parallel"""
    ...
    def process_region(self, region: str):
        ...
        # Each thread gets its own collector and activator (not thread-safe to share)
        collector = NexusCollector()
        activator = NexusActivator()
        
        region_players = 0
        
        print(f"[NEXUS:{region.upper()}] Thread started")
        
        try:
            while datetime.now() < self.end_time and not self.should_stop:
                # Get next player from NEXUS queue (auto-resolves PUUID if needed)
                player = activator.get_next_ready_player(region=region)
                
                if not player:
                    print(f"[NEXUS:{region.upper()}] Queue empty, waiting 30 seconds...")
                    time.sleep(30)
                    continue
                
                # Mark as processing
                mark_queue_processing(player.region, player.game_name, player.tag_line, 'nexus')
                ...
                try:
                    ...
                    if stats['success']:
                        mark_queue_complete(player.region, player.game_name, player.tag_line, 'nexus')
                    ...
                    print(f"[NEXUS:{region.upper()}] ❌ Error processing {player.game_name}#{player.tag_line}: {e}")
                    mark_queue_error(
                        player.region,
                        player.game_name,
                        player.tag_line,
            ...
            collector.close()
            activator.close()
            print(f"[NEXUS:{region.upper()}] Thread finished - {region_players} players processed")
    
    def run(self):
        """Run parallel collection"""
        print("="*60)
        print("NEXUS PARALLEL COLLECTION RUNNER")
        print("="*60)
        ...
        # Initialize database
        db_pool.initialize()
        mark_service_active('nexus', 'collecting_parallel')
        ...
        try:
            ...
            print("\n" + "="*60)
            print("NEXUS COLLECTION COMPLETE")
            print("="*60)
            ...
            print("\n✅ NEXUS parallel collection runner finished!")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='Run NEXUS collection in parallel for all regions')
    parser.add_argument(
        '--hours',
        type=float,
        default=5.0,
