"""
Neural Nexus v3.0 - APEX Collector
Collects 200 games of match history for each Challenger player
"""
...
class ApexCollector:
    ...
    def collect_player_matches(self, region: str, game_name: str, tag_line: str) -> dict:
        ...
        if not player.apex_puuid:
            return {'success': False, 'error': 'Player missing APEX PUUID'}
        
        print(f"\n{'='*60}")
        print(f"[{region.upper()}] Collecting matches for {game_name}#{tag_line}")
        print(f"{'='*60}")
        ...
        update_apex_check(region, game_name, tag_line, match_count=stats['matches_collected'])
    ...
    def _collect_single_match(self, match_id: str, region: str) -> dict:
        ...
        # Try to acquire lock
        if not match_lock.try_acquire(match_id, 'apex'):
...
if __name__ == "__main__":
    ...
    if not players:
        print("❌ No players in APEX queue. Run seeder first!")
        db_pool.close()
        exit(1)
